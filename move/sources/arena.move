...
//! ctx (transaction context yani işlem bağlamı) Bir fonksiyonun blockchain üzerinde çalıştığı sırada, işlemin kim tarafından yapıldığını ve hangi kaynaklara erişebileceğini tutan yapı.
    let arena = Arena { //Arena'yı tanımladığımız yer.
        id: object::new(ctx),
        warrior: hero,
        owner: tx::sender(ctx),
    };
    event::emit(ArenaCreated { //Arenayı event olarak oluşturduğumuz ve blockchain üzerinde kaydını tuttuğumuz yer.
